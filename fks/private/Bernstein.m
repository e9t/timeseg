...
try
    ...
    ME = lasterror();  %#ok % use lasterror is backward compatible with 2006B
    % Edge vector not monotonically non-decreasing.
    if strmatch('MATLAB:histc:InvalidInput',ME.identifier)
        B = [];
        return
    else
        rethrow(ME)
