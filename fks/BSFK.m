function [pp ier RMS] = BSFK(x, y, k, nknots, fixknots, options)
%
% pp = BSFK(x, y)
...
while true
    ...
    if graphic
        ...
        plot(ax2, unnormfun(x), y, '.b','MarkerSize',bigmarkersize);
        hold(ax2,'on');
        graph_hyfit = plot(ax2, unnormfun(x), yfit, 'r');
        yl = ylim(ax2);
        plot(ax2, unnormfun(t(1)*[1 1]), yl, 'g-.');
        plot(ax2, unnormfun(t(end)*[1 1]), yl, 'g-.');
        graph_htp = zeros(size(p));
        for j=reshape(p,1,[])
            graph_htp(j) = plot(ax2, unnormfun(t(j)*[1 1]), yl, 'g-.');
        end
...
if graphic    
    ...
    plot(ax2, unnormfun(t(end)*[1 1]), yl, 'g-.');
    for j=reshape(p,1,[])
        plot(ax2, unnormfun(t(j)*[1 1]), yl, 'g-.');
    end
    for j=1:length(fknt)
        plot(ax2, unnormfun(fknt(j)*[1 1]), yl, 'c-');
    end
